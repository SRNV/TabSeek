...
<template>
  <div class="chord-tab-container">
    ...
    <div class="chord-content">
      ...
      <Notes :key="store.chordRootNoteType + store.chordRootNote" :rootNote="store.chordRootNote" :notes="chordNotes"
        :collection="modeNotes" :chordType="store.chordRootNoteType" />

      <!-- Utilisation de Tab.vue pour afficher l'accord sur le manche -->
      <div class="tab-wrapper">
        <Tab :midiList="chordMidiList" matchType="one" :tabLength="tabLength" :visibleStart="visibleStart"
      ...
      <div class="chord-intervals" v-if="chordData.intervals && chordData.intervals.length > 0">
        ...
        <div class="intervals-list">
          <span v-for="(interval, index) in chordData.intervals" :key="interval" class="interval-chip"
      ...
      <div class="chord-fingering" v-if="chordData.positions && chordData.positions.length > 0">
        ...
        <div class="fingering-list">
          <div v-for="(position, posIndex) in chordData.positions.slice(0, 1)" :key="posIndex" class="position-card">
            <div class="position-number">Position {{ position.position || posIndex + 1 }}</div>
            <div class="position-frets">
              <span v-for="(fret, fretIndex) in position.frets" :key="fretIndex" class="fret-value"
