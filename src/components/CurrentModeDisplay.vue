<!-- CurrentModeDisplay.vue - Composant utilitaire pour afficher le mode actuel -->
<template>
    <div class="current-mode-info" v-if="currentMode">
      <div class="mode-header">
        <h4 class="mode-name">{{ currentMode.name }}</h4>
        <div class="mode-scale">{{ scaleInfo }}</div>
      </div>
      <div class="mode-culture" v-if="currentMode.culture">
        <span class="culture-label">Culture:</span> {{ currentMode.culture }}
      </div>
      <div class="mode-description" v-if="currentMode.description || modeDescription">
        {{ currentMode.description || modeDescription }}
      </div>
      <div class="mode-intervals">
        <span class="interval-title">Intervalles:</span>
        <div class="interval-list">
          {{ currentMode.intervals.join(', ') }}
        </div>
      </div>
      <div class="mode-chords">
        <div class="chord-type">
          <span class="chord-label">Accord triade:</span> 
          <span class="chord-value">{{ currentMode.triad }}</span>
        </div>
        <div class="chord-type">
          <span class="chord-label">Accord septième:</span> 
          <span class="chord-value">{{ currentMode.seventh }}</span>
        </div>
      </div>
      <div class="mode-notes" v-if="modeNotes.length > 0">
        <span class="notes-title">Notes:</span>
        <div class="notes-list">
          <span class="mode-note" v-for="(note, idx) in modeNotes" :key="idx">{{ note }}</span>
        </div>
      </div>
    </div>
  </template>
